...
const statusVariant = (status: string) => (status === 'verified' ? 'default' : status === 'rejected' ? 'destructive' : 'secondary');
            ...
            <>
                ...
                <div className="flex h-full flex-1 items-center justify-center p-4">
                    <Card className="max-w-md p-8 text-center">
                        ...
                        <p className="text-sm text-muted-foreground">Akun Anda belum terhubung dengan data peserta. Silakan hubungi admin atau daftar olimpiade.</p>
                    </Card>
                </div>
            </>
        );
            ...
            <div className="flex flex-1 flex-col gap-6 p-4">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <div className="mt-1 flex items-center gap-2">
                            ...
                            <Badge variant={statusVariant(participant.status) as any}>{labels[participant.status] ?? participant.status}</Badge>
                        </div>
                    </div>
                </div>
                <div className="grid gap-6 lg:grid-cols-[1fr_0.8fr]">
                    ...
                    <Card className="space-y-5 p-5">
                        ...
                        <Detail label="Referensi" value={participant.referral_source} />
...
const FileLink = ({ label, href }: { label: string; href?: string | null }) => (
    <div>
        <p className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">{label}</p>
        {href ? (
            <a href={href} target="_blank" rel="noopener noreferrer" className="mt-1 inline-flex items-center gap-2 text-sm font-semibold text-primary underline">
                Lihat file <ExternalLink className="h-4 w-4" />
            </a>
        ) : (
