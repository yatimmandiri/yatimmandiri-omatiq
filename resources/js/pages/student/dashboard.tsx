                ...
                <div className="flex min-h-[60vh] items-center justify-center p-6">
                    <Card className="max-w-md rounded-[2rem] p-8 text-center shadow-sm">
                        ...
                        <p className="mt-2 text-sm leading-6 text-muted-foreground">
                            ...
                            Daftar olimpiade atau hubungi admin.
                        </p>
                        <a
                            href="/olimpiade"
                            className="mt-6 inline-flex rounded-xl bg-[#17524A] px-5 py-2.5 text-sm font-bold text-white"
                        >
                            Lihat Olimpiade
                        </a>
                    </Card>
        ...
        <>
            ...
            <div className="flex flex-1 flex-col gap-6 p-4 lg:p-6">
                ...
                <div className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#17524A] to-[#0e2e28] p-6 text-white shadow-xl lg:p-7">
                    <div className="absolute -top-10 -right-10 size-64 rounded-full bg-[#E5BE1E]/15 blur-3xl" />
                    ...
                    <div className="relative flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div>
                            ...
                            <p className="mt-1 flex flex-wrap items-center gap-2 text-sm text-white/70">
                                ...
                                <Badge
                                    ...
                                    className="ml-1 bg-white text-[#17524A] hover:bg-white"
                                >
                        ...
                        <div className="flex items-center gap-3 rounded-2xl bg-white/10 p-3 backdrop-blur">
                            <div
                                className={`flex size-10 items-center justify-center rounded-xl ${isVerified ? 'bg-[#E5BE1E] text-[#17524A]' : 'bg-white/15 text-white'}`}
                            >
                ...
                <Card className="rounded-[1.5rem] p-5">
                    <div className="flex items-center gap-2 text-sm font-black">
                        <Award className="size-4 text-[#17524A]" /> Status
                        Pendaftaran
                    </div>
                    <div className="mt-4 grid grid-cols-3 gap-2">
                        {[
                            ...
                            return (
                                <div
                                    key={i}
                                    className={`rounded-2xl p-3 text-center text-xs font-bold ${done ? 'bg-[#17524A] text-white' : 'bg-zinc-100 text-muted-foreground dark:bg-zinc-800'}`}
                                >
                                    <div
                                        className={`mx-auto mb-1 flex size-6 items-center justify-center rounded-full text-[11px] ${done ? 'bg-[#E5BE1E] text-[#17524A]' : 'bg-white dark:bg-zinc-700'}`}
                                    >
                                        {i + 1}
                                    </div>
                                    {s.label}
                ...
                <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
                    <Card className="rounded-[1.5rem] p-6">
                        <h2 className="flex items-center gap-2 text-base font-black">
                            <FileText className="size-4 text-[#17524A]" />{' '}
                            Biodata
                        </h2>
                        <div className="mt-5 grid gap-4 sm:grid-cols-2">
                            ...
                            <Detail
                                label="TTL"
                                value={`${participant.student?.birth_place ?? ''}${participant.student?.birth_date ? ', ' + participant.student?.birth_date.slice(0, 10) : ''}`}
                            />
                            <Detail
                                label="Sekolah"
                    ...
                    <div className="flex flex-col gap-6">
                        <Card className="rounded-[1.5rem] bg-gradient-to-br from-[#E5BE1E]/15 to-transparent p-6">
                            <h2 className="flex items-center gap-2 text-base font-black">
                                <GraduationCap className="size-4 text-[#17524A]" />{' '}
                                Olimpiade
                            </h2>
                            <p className="mt-2 text-lg font-black text-[#17524A]">
                                {participant.olimpiade?.name ?? '-'}
                            </p>
                        ...
                        <Card className="rounded-[1.5rem] p-6">
                            ...
                            <div className="mt-4 grid gap-3">
                                ...
                                <Doc
                                    label="Identitas / KK"
                                    href={
                                        participant.student?.identity_card_url
                                    }
                                />
                            </div>
                        </Card>
...
const Detail = ({
    ...
    <div className={full ? 'sm:col-span-2' : ''}>
        <p className="flex items-center gap-1 text-[11px] font-black tracking-widest text-muted-foreground uppercase">
            {icon} {label}
        </p>
...
const Doc = ({ label, href }: { label: string; href?: string | null }) => (
    <a
        ...
        className={`flex items-center justify-between rounded-2xl border p-3 text-sm font-semibold transition ${href ? 'bg-white hover:bg-zinc-50 dark:bg-zinc-900' : 'bg-zinc-50 opacity-60 dark:bg-zinc-800'}`}
    >
        <span>{label}</span>
        <span
            className={`rounded-full px-2.5 py-1 text-xs font-black ${href ? 'bg-[#17524A] text-white' : 'bg-zinc-200 dark:bg-zinc-700'}`}
        >
            {href ? 'Lihat' : '-'}
        </span>
    </a>
