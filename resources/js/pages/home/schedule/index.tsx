...
> = {
    registration: { bg: '#E7F0ED', text: '#17524A', icon: CalendarDays },
    technical_meeting: { bg: '#FBF4DC', text: '#17524A', icon: Clock3 },
    preliminary: { bg: '#FBF4DC', text: '#9C7A0A', icon: CheckCircle2 },
    knockout: { bg: '#ECFDF5', text: '#12885B', icon: Flag },
    semifinal: { bg: '#F3E8FF', text: '#8B5CF6', icon: Medal },
    final: { bg: '#FBF4DC', text: '#17524A', icon: Trophy },
    announcement: { bg: '#E7F0ED', text: '#17524A', icon: Sparkles },
};
...
const defaultSchedules: ScheduleItem[] = [
    {
        ...
        description:
            "Sekolah, orang tua, dan pendamping mulai mendaftarkan peserta untuk cabang Al-Qur'an dan Matematika.",
        color: '#17524A',
        actionLabel: 'Daftar',
        actionUrl: '/kontak',
        olimpiade: null,
    },
    {
        ...
        description:
            'Peserta mengikuti seleksi awal sesuai cabang lomba untuk menentukan peserta yang lanjut ke fase gugur.',
        color: '#E5BE1E',
        olimpiade: null,
    },
    {
        ...
        location: 'Online Nasional',
        description:
            'Peserta terbaik bertanding dalam fase gugur hingga tersisa finalis nasional.',
        color: '#5DD39E',
        olimpiade: null,
    },
    {
        ...
        description:
            'Finalis tampil di panggung puncak OMATIQ bersama peserta terbaik dari berbagai daerah.',
        color: '#17524A',
        olimpiade: null,
    },
];
...
export default function SchedulePage() {
    ...
    const schedules = props.schedules?.length
        ? props.schedules
        : defaultSchedules;
    ...
    return (
        <>
            <section className="relative overflow-hidden px-5 pt-28 pb-14 sm:pt-32 sm:pb-20 lg:px-8">
                <div className="absolute top-24 left-0 h-56 w-56 rounded-[56px] bg-[#17524A]/15 blur-3xl" />
                ...
                <div className="relative mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-[0.95fr_1.05fr]">
                    <div>
                        ...
                        <h1 className="mt-6 max-w-4xl text-3xl leading-tight font-black text-[#1E293B] sm:text-5xl lg:text-7xl">
                            Dari registrasi sampai final, semua fase tertata
                            jelas.
                        </h1>
                        <p className="mt-5 max-w-2xl text-base leading-8 text-[#64748B] sm:text-lg">
                            Pantau perjalanan Olimpiade Al-Qur'an dan Matematika
                            OMATIQ dalam satu kalender yang mudah dibaca oleh
                            peserta, orang tua, guru, dan pendamping.
                        </p>
                        <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                            <a
                                href="#calendar"
                                className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#17524A] px-6 py-4 text-sm font-black text-white shadow-xl shadow-[#17524A]/25 transition hover:-translate-y-1"
                            >
                                Lihat Kalender
                            ...
                            <Link
                                href="/olimpiade"
                                className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#17524A]/15 bg-white px-6 py-4 text-sm font-black text-[#17524A] shadow-sm transition hover:-translate-y-1"
                            >
                                Pilih Olimpiade
                    ...
                    <div className="relative">
                        <div className="rounded-[32px] bg-white p-5 shadow-2xl ring-1 shadow-[#17524A]/10 ring-slate-100">
                            <div className="rounded-[26px] bg-gradient-to-br from-[#17524A] via-[#17524A] to-[#56CCF2] p-5 text-white sm:p-7">
                                ...
                                <div className="mt-8 grid gap-3 sm:grid-cols-2">
                                    <InfoPill
                                        icon={Clock3}
                                        label="Tanggal"
                                        value={heroSchedule?.dateLabel ?? '-'}
                                    />
                                    <InfoPill
                                        icon={MapPin}
                                        label="Lokasi"
                                        value={heroSchedule?.location ?? '-'}
                                    />
                                </div>
                                <div className="mt-6 rounded-3xl bg-white/12 p-5 backdrop-blur">
                                    <p className="text-sm leading-7 text-white/80">
                                        {heroSchedule?.description ??
                                            'Tim OMATIQ akan mengumumkan kalender resmi melalui halaman ini.'}
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div className="absolute -right-4 -bottom-4 hidden rounded-3xl bg-[#E5BE1E] px-5 py-4 text-sm font-black text-[#1E293B] shadow-xl sm:block">
                            Siap dari awal
                        </div>
                    </div>
                </div>
            ...
            <section
                id="calendar"
                className="bg-white px-5 py-14 sm:py-20 lg:px-8"
            >
                                ...
                                {Object.entries(grouped).map(
                                    ([phase, items]) => (
                                        <a
                                            ...
                                            className="flex items-center justify-between rounded-2xl bg-white p-4 text-sm font-black text-[#1E293B] shadow-sm ring-1 ring-slate-100 transition hover:-translate-y-0.5 hover:text-[#17524A]"
                                        >
                                            <span>{phase}</span>
                                            <span className="rounded-full bg-[#17524A]/10 px-3 py-1 text-xs text-[#17524A]">
                                                {items.length}
                                            </span>
                                        </a>
                                    ),
                        ...
                        <div className="relative">
                            ...
                            <div className="space-y-5">
                                {schedules.map((item, index) => {
                                    const tone =
                                        phaseTone[item.phase] ??
                                        phaseTone.registration;
                                    ...
                                    return (
                                        <article
                                            id={item.phaseLabel
                                                ...
                                                .replace(/\s+/g, '-')}
                                            key={item.id}
                                            className="group relative grid gap-4 rounded-[28px] bg-[#F8FAFC] p-4 ring-1 ring-slate-100 transition duration-300 hover:-translate-y-1 hover:bg-white hover:shadow-2xl hover:shadow-[#17524A]/10 sm:grid-cols-[auto_1fr] sm:p-5"
                                        >
                                            <div
                                                className="relative z-10 flex h-12 w-12 items-center justify-center rounded-2xl text-white shadow-lg"
                                                style={{
                                                    backgroundColor:
                                                ...
                                                <div className="flex flex-wrap items-center gap-2">
                                                    ...
                                                    {item.olimpiade && (
                                                        <Link
                                                            ...
                                                            className="rounded-full bg-white px-3 py-1 text-xs font-black text-[#17524A] ring-1 ring-slate-100"
                                                        >
                                                            {
                                                                item.olimpiade
                                                                    .title
                                                ...
                                                {item.actionUrl &&
                                                    item.actionLabel && (
                                                        <Link
                                                            ...
                                                            className="mt-5 inline-flex items-center gap-2 rounded-xl bg-[#17524A] px-5 py-3 text-sm font-black text-white transition hover:-translate-y-0.5"
                                                        >
                                                            {item.actionLabel}
                                                            <ArrowRight className="h-4 w-4" />
                                                        </Link>
