...
export default function CreatePage() {
    ...
    const error = (n: string) =>
        form.errors[n] ? (
            <p className="text-sm text-destructive">{form.errors[n]}</p>
        ) : null;
    ...
    return (
        <form onSubmit={submit} className="space-y-6 p-4">
            ...
            <Card className="space-y-5 p-5">
                <Field label="NIK" error={error('nik')}>
                    <Input
                        value={form.data.nik}
                        onChange={(e) =>
                            form.setData(
                ...
                <div className="grid gap-5 md:grid-cols-2">
                    ...
                    <Field label="Kabupaten" error={error('regency_id')}>
                        <select
                ...
                <div className="grid gap-5 md:grid-cols-2">
                    ...
                    <Field label="Desa" error={error('village_id')}>
                        <select
                            value={form.data.village_id}
                            onChange={(e) =>
                                form.setData('village_id', e.target.value)
                            }
                            className="w-full rounded-md border px-3 py-2 text-sm"
                            disabled={
                                !form.data.district_id || isLoadingVillages
                            }
                            required
                        >
                            <option value="">
