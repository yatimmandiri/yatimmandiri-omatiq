...
import { ArrowLeft, Save, User, GraduationCap, MapPin, Phone, ShieldCheck } from 'lucide-react';
    ...
    return (
        <form onSubmit={submit} className="mx-auto max-w-5xl space-y-6 p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 className="text-2xl font-bold tracking-tight">Edit Data Binaan / Student</h1>
                    <p className="text-sm text-muted-foreground">
                        Perbarui data master santri/siswa ({student.full_name}). Sinkronisasi ke Penyaluran otomatis jika terhubung.
                    </p>
                </div>
            ...
            <Card>
                <CardHeader className="pb-3">
                    ...
                    <CardDescription>Informasi identitas dasar siswa/santri.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <Field label="NIS (Nomor Induk Siswa)" error={error('nis')}>
                            <Input
                                value={form.data.nis}
            ...
            <Card>
                <CardHeader className="pb-3">
                    ...
                    <CardDescription>Informasi sekolah dan jenjang pendidikan.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
            ...
            <Card>
                <CardHeader className="pb-3">
                    <div className="flex items-center gap-2">
                        ...
                        <CardTitle className="text-base font-semibold">Alamat & Wilayah</CardTitle>
                    </div>
                    <CardDescription>Lokasi domisili santri/siswa.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
            ...
            <Card>
                <CardHeader className="pb-3">
                    <div className="flex items-center gap-2">
                        ...
                        <CardTitle className="text-base font-semibold">Kontak & Pembimbing</CardTitle>
                    </div>
                    <CardDescription>Nomor kontak orang tua/wali dan guru pembimbing.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="grid gap-4 md:grid-cols-2">
                        <Field label="No. WhatsApp / HP Orang Tua (Wali) *" error={error('parent_phone')}>
                            <Input
                        ...
                        <Field label="Guru Pembimbing / Mentor" error={error('mentor_id')}>
                            <select
                                value={form.data.mentor_id}
                                onChange={(e) => {
                                    const selectedId = e.target.value;
                                    const mentorObj = mentors.find((m) => String(m.id) === selectedId);
                                    form.setData('mentor_id', selectedId);
                                    if (mentorObj) {
                                        form.setData('mentor_name', mentorObj.name);
                                    }
                                }}
                                className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm shadow-xs focus:outline-none focus:ring-2 focus:ring-ring"
                            >
            ...
            <Card>
                <CardHeader className="pb-3">
                    <div className="flex items-center gap-2">
                        ...
                        <CardTitle className="text-base font-semibold">Status & Pengaturan</CardTitle>
                    </div>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="grid gap-4 md:grid-cols-2">
                        <div className="flex items-center justify-between rounded-lg border p-4">
                            <div className="space-y-0.5">
                                <Label className="text-base">Kategori Binaan</Label>
                                <p className="text-xs text-muted-foreground">
                                    {form.data.is_binaan ? 'Santri Binaan Yatim Mandiri' : 'Peserta Umum / Non-Binaan'}
                                </p>
                            </div>
                            <Switch
                                checked={form.data.is_binaan}
                                onCheckedChange={(v) => form.setData('is_binaan', v)}
                            />
                        </div>

                        <div className="flex items-center justify-between rounded-lg border p-4">
                            <div className="space-y-0.5">
                                <Label className="text-base">Status Akun / Master</Label>
                                <p className="text-xs text-muted-foreground">
                                    {form.data.is_active ? 'Aktif dapat didaftarkan' : 'Non-aktif'}
                                </p>
                            </div>
                            <Switch
