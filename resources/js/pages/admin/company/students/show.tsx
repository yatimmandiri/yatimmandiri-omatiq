        ...
        <div className="space-y-6 p-4">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                        {student.nik} {student.nis ? `• ${student.nis}` : ''}
                    </p>
                </div>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
                <Card className="space-y-4 p-5">
                    ...
                    <Detail
                        label="Wilayah"
                        value={`${student.province?.name ?? ''} ${student.regency?.name ?? ''}`}
                    />
                    <Detail
                        label="Mentor"
                        value={student.mentor?.name ?? student.mentor_name}
                    />
                    <Detail label="HP Wali" value={student.parent_phone} />
                    <Detail
                        label="Penyaluran ID"
                        value={student.penyaluran_id}
                    />
                </Card>

                <Card className="space-y-4 p-5">
                    <h2 className="text-lg font-semibold">
                        Peserta Terdaftar ({participants?.length ?? 0})
                    </h2>
                    {participants?.length ? (
