...
import sanggarRoute from '@/routes/admin/data-sanggar';
...
type PageProps = {
    ...
    sanggars?: Array<{ id: string | number; name: string; type?: string; total_students?: number }>;
...
export default function Dashboard() {
    ...
    const userName = (auth as any)?.user?.name ?? biodata?.name ?? penyaluranProfile?.name ?? 'Guru';
    const hour = new Date().getHours();
    const greeting = hour < 11 ? 'Selamat Pagi' : hour < 15 ? 'Selamat Siang' : hour < 19 ? 'Selamat Sore' : 'Selamat Malam';
    const progressWidth = `${percent}%`;
    const registrationRate = totalBinaan ? Math.round((totalRegistered / totalBinaan) * 100) : 0;
        ...
        <>
            ...
            <div className="flex flex-1 flex-col gap-6 p-4 lg:p-6">
                ...
                <div className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#17524A] via-[#13423c] to-[#0e2e28] p-6 text-white shadow-xl lg:p-8">
                    <div className="absolute -right-16 -top-16 size-72 rounded-full bg-[#E5BE1E]/15 blur-3xl" />
                        ...
                        <div className="flex gap-4">
                            ...
                            <div>
                                ...
                                <p className="mt-2 max-w-xl text-sm leading-6 text-white/70">
                                    Kelola binaan, pantau sanggar, dan selesaikan pendaftaran OMATIQ 2026 dalam satu tempat.
                                    {penyaluranProfile?.kantor_name ? ` • ${penyaluranProfile.kantor_name}` : ''}
                                </p>
                                <div className="mt-4 flex flex-wrap gap-2">
                                    <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1 text-xs font-semibold backdrop-blur">
                                        ...
                                        {sanggarSum != null ? ` • ${sanggarSum} santri` : ''}
                                    </span>
                                    ...
                                    <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1 text-xs font-semibold backdrop-blur">
                                        <CheckCircle2 className="size-3.5 text-emerald-300" /> {registrationRate}% terdaftar
                                    </span>
                                </div>
                                {/* mini progress */}
                                <div className="mt-4 flex items-center gap-3">
                                    <div className="h-1.5 w-36 overflow-hidden rounded-full bg-white/15">
                                        <div className="h-full rounded-full bg-[#E5BE1E] transition-all" style={{ width: `${registrationRate}%` }} />
                                    </div>
                                    <span className="text-xs font-bold text-[#E5BE1E]">{totalRegistered}/{totalBinaan} terdaftar</span>
                                </div>
                            </div>
                        </div>
                        <div className="flex shrink-0 flex-col gap-2 sm:flex-row lg:flex-col">
                            <Link href={binaan.index().url} prefetch className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#E5BE1E] px-5 py-3 text-sm font-black text-[#17524A] shadow-lg shadow-black/10 transition hover:translate-y-[-1px] hover:bg-[#d9b01c]">
                                Kelola Binaan <ArrowRight className="size-4" />
                            </Link>
                            <Link href="/teacher/biodata" prefetch className="inline-flex items-center justify-center gap-2 rounded-2xl bg-white/10 px-5 py-2.5 text-sm font-semibold text-white backdrop-blur transition hover:bg-white/15">
                                <BookOpenCheck className="size-4" /> {percent === 100 ? 'Lihat Biodata' : 'Lengkapi Biodata'}
                            </Link>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    ...
                    <StatCard icon={<Building2 />} label="Total Sanggar" value={sanggarCount ?? 0} hint={sanggarSum != null ? `${sanggarSum} santri total` : 'Unit binaan'} accent="#E5BE1E" darkText />
                    <StatCard icon={<CheckCircle2 />} label="Sudah Terdaftar" value={totalRegistered} hint={`${registrationRate}% dari total`} accent="#22c55e" />
                        ...
                        <div className="grid gap-6 p-6 lg:grid-cols-[1.15fr_0.85fr] lg:p-7">
                            <div className="flex gap-5">
                                ...
                                <div className="min-w-0 flex-1">
                                    ...
                                    {!biodataCompleteness.is_complete && biodataCompleteness.missing.length > 0 && (
                                        <div className="mt-2 flex flex-wrap gap-1.5">
                                            {biodataCompleteness.missing.slice(0, 5).map((f) => (
                                                <Badge key={f} variant="secondary" className="rounded-full text-[10px] capitalize">
                                                    {f.replace('_id', '').replace('_', ' ')}
                                                </Badge>
                                            ))}
                                            {biodataCompleteness.missing.length > 5 && (
                                                <Badge variant="outline" className="rounded-full text-[10px]">
                                                    +{biodataCompleteness.missing.length - 5} lagi
                                                </Badge>
                                            )}
                                        </div>
                                    )}
                            ...
                            <div className="flex flex-col justify-center gap-3 rounded-2xl bg-[#17524A]/[0.06] p-4 dark:bg-white/5">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    ...
                                    <Link href={binaan.index().url} prefetch className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-semibold shadow-sm ring-1 ring-zinc-200 hover:bg-zinc-50 dark:bg-zinc-800 dark:ring-zinc-700">
                                        Lihat Binaan
                                    </Link>
                                </div>
                                <p className="text-xs leading-4 text-muted-foreground">Data tersinkron langsung ke Penyaluran — single source of truth. Foto & NIK valid mempercepat verifikasi.</p>
                            </div>
                        </div>
                    </Card>
                )}
                ...
                <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
                    ...
                    <Card className="rounded-[1.75rem] p-6 shadow-sm">
                        <div className="flex items-center justify-between">
                            <h2 className="flex items-center gap-2 text-base font-black">
                                <Users className="size-4 text-[#17524A]" /> Binaan Terbaru
                            </h2>
                            <Link href={binaan.index().url} className="text-xs font-bold text-[#17524A] hover:underline">
                                Lihat semua →
                            </Link>
                        </div>
                        {penyaluranStudents.length === 0 ? (
                            <div className="mt-6 rounded-2xl border border-dashed p-8 text-center">
                                ...
                                <p className="mt-3 text-sm font-bold">Belum ada data Penyaluran</p>
                                <p className="mt-1 text-xs text-muted-foreground">Pastikan token Penyaluran valid atau tambahkan binaan manual.</p>
                                <Link href={binaan.create().url} className="mt-4 inline-flex rounded-xl bg-[#17524A] px-4 py-2 text-sm font-bold text-white">
                                    Tambah Binaan
                                </Link>
                            </div>
                        ) : (
                            <div className="mt-4 grid gap-3">
                                {penyaluranStudents.slice(0, 5).map((s: any, i: number) => (
                                    <div key={`${s.student_id ?? s.id ?? i}-${i}`} className="flex items-center justify-between gap-3 rounded-2xl border p-3 transition hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                                        <div className="flex items-center gap-3 min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                <p className="flex items-center gap-1 truncate text-xs text-muted-foreground">
                                                    <MapPin className="size-3 shrink-0" /> {s.sanggar_name ?? s.school_name ?? '-'}
                                                </p>
                                            </div>
                                        </div>
                                        <Badge variant="secondary" className="shrink-0 rounded-full">{s.status ?? 'aktif'}</Badge>
                                    </div>
                                ))}
                            </div>
                        )}
                    ...
                    <div className="flex flex-col gap-6">
                        <Card className="rounded-[1.75rem] bg-gradient-to-br from-[#E5BE1E]/15 via-transparent to-[#17524A]/5 p-6">
                            <h2 className="flex items-center gap-2 text-base font-black">
                                <Building2 className="size-4 text-[#17524A]" /> Sanggar
                            </h2>
                            {sanggars.length === 0 ? (
                                <p className="mt-3 text-sm text-muted-foreground">Belum ada sanggar terhubung.</p>
                            ) : (
                                <div className="mt-4 grid gap-2">
                                    {sanggars.slice(0, 3).map((sg) => (
                                        <div key={String(sg.id)} className="flex items-center justify-between rounded-2xl bg-white p-3 text-sm shadow-sm dark:bg-zinc-900">
                                            ...
                                            <span className="rounded-full bg-[#17524A]/10 px-2.5 py-1 text-xs font-bold text-[#17524A]">
                                                {sg.total_students != null ? `${sg.total_students} santri` : sg.type ?? 'sanggar'}
                                            </span>
                                        </div>
                                    ))}
                                    {sanggars.length > 3 && <p className="text-xs text-muted-foreground">+{sanggars.length - 3} sanggar lainnya</p>}
                                </div>
                            )}
                            <Link href={sanggarRoute.index().url} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#17524A] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#0f3d36]">
                        ...
                        <Card className="rounded-[1.75rem] border-dashed p-6">
                            ...
                            <div className="mt-4 grid gap-3">
                                <QuickAction title="Daftarkan Binaan" desc="Pilih binaan → olimpiade" href={dataPeserta.create().url} icon={<BookOpenCheck className="size-4" />} accent="#17524A" />
                                <QuickAction title="Data Peserta" desc="Cek status pendaftaran" href={dataPeserta.index().url} icon={<CheckCircle2 className="size-4" />} accent="#E5BE1E" dark />
                                <QuickAction title="Absensi" desc="Rekap kehadiran santri" href="/admin/absensi" icon={<Award className="size-4" />} accent="#0ea5e9" />
                            </div>
                        </Card>
                    </div>
                </div>
                        ...
                        <div className="flex gap-4">
                            ...
                            <div>
                                ...
                                <p className="mt-1 max-w-xl text-sm leading-6 text-muted-foreground">
                                    Pilih binaan yang <span className="font-semibold text-[#17524A]">belum terdaftar</span>, pilih kategori olimpiade, dan submit — status otomatis <span className="font-semibold text-emerald-600">verified</span> karena sudah terverifikasi Penyaluran.
                                </p>
                            </div>
                        </div>
                        <Link href={binaan.index().url} prefetch className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-[#17524A] px-5 py-3 text-sm font-black text-white shadow hover:bg-[#0f3d36]">
...
const QuickAction = ({ title, desc, href, icon, accent, dark }: { title: string; desc: string; href: string; icon: ReactNode; accent: string; dark?: boolean }) => (
    <Link href={href} prefetch className="group flex items-center gap-3 rounded-2xl border bg-white p-3 transition hover:bg-zinc-50 dark:bg-zinc-900 dark:hover:bg-zinc-800">
        <div className="flex size-10 items-center justify-center rounded-xl text-white" style={{ background: accent, color: dark ? '#17524A' : 'white' }}>
            {icon}
        </div>
        <div className="min-w-0 flex-1">
